...
from lib.get_frame import get_per_frame_from_dataset
from lib.yaw_and_offset import post_processing
...
class LaneDataset(Dataset):
    def __init__(self,
        ...
        self.index = 0
        self.file_name = '/mnt/synchronization/capture6/left/'
    ...
    def draw_annotation(self, idx, matrix_ipm, calib_points_dict, intersect_dict, label=None, pred=None, img=None):
        # Get image if not provided
        if img is None:
            # print(self.annotations[idx]['path'])
            img, label, imageName = self.__getitem__(idx)
            ...
            _, label, imageName = self.__getitem__(idx, next=True)
            label = self.label_to_lanes(label)
        ...
        camYaw, camBias = post_processing(img, laneDict, imageName, matrix_ipm, calib_points_dict, intersect_dict)
            # joblib.dump(laneDict, os.path.join("test_points5", '%s.pkl') % imageName.split(".")[0])
    ...
    def __getitem__(self, idx, next=False):
        item = self.dataset[idx]
        # img_org = cv2.imread(item['path'])

        img_org, img_name = get_per_frame_from_dataset(self.file_name, self.index)
        if next:
