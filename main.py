...
def main():
    ...
    # 实测仿真：save_predictions=False，测试：save_predictions=args.save_predictions
    # runner.eval(epoch=args.epoch or exp.get_last_checkpoint_epoch(), save_predictions=args.save_predictions)
    runner.eval(epoch=args.epoch or exp.get_last_checkpoint_epoch(), save_predictions=False)
