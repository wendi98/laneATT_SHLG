...
class LaneEval(object):
    ...
    def bench(pred, gt, y_samples, running_time, get_matches=False):
        ...
        for x_gts, thresh in zip(gt, threshs):
            ...
            # with open('acc.json', "a", encoding="utf-8") as f:
            #     json.dump(accs, f)
            #     f.write('\n')
        ...
        return s / max(min(4.0, len(gt)), 1.), fp / len(pred) if len(pred) > 0 else 0., fn / max(
            min(len(gt), 4.), 1.), matched / len(pred) if len(pred) > 0 else 0.

    @staticmethod
    def bench_one_submit(pred_file, gt_files):
        try:
        ...
        for gt_file in gt_files:
            json_gt.extend(json.loads(line) for line in open(gt_file).readlines())
        ...
        for pred in json_pred:
            ...
            try:
                a, p, n, t = LaneEval.bench(pred_lanes, gt_lanes, y_samples, run_time)
            except BaseException as e:
                raise Exception('Format of lanes error.')
            accuracy += a
            fp += p
